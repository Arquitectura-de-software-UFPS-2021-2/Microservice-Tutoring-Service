...
@Validated
@CrossOrigin(origins = "*")
public class EndPointListarTutoriasTrue {

    @Autowired
    private ManejadorListarTutoriasTrue manejadorListarTutoriasTrue;

    //---GUARDAR---
    @GetMapping(REST_LIST+REST_TUTORIA_LISTAR_TRUE)
    @ApiOperation("listar una tutorias activas")
    @ApiResponses({
    ...
    public ResponseEntity<List<TutoriaSalida>> getAll() throws NotFoundException {
        List<TutoriaSalida> tutorias = manejadorListarTutoriasTrue.list();
        if (tutorias.isEmpty()){
